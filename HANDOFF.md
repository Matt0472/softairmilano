...
## Obiettivo
Nuovo sito **softairmilano.it** (arena indoor di softair a Milano ~1.500 mq) in **Astro statico + Tailwind v4 + Pages CMS**, deploy su **Cloudflare Pages**. Sostituisce il vecchio sito WordPress su Aruba.
...
- **Home rifatta con impatto visivo forte.** Ordine sezioni: **Hero → L'arena → Video → (Attività/Occasioni/Ranger/CTA già presenti dalla vecchia Home, da ridisegnare in coerenza)**.
  ...
  - **Video** (`VideoSection.astro`): sezione **split** (desktop copy + video HUD; mobile scritta sopra, video sotto). Embed **YouTube** ripulito (nocookie, controls=0, rel=0, autoplay muto loop, crop per nascondere il chrome, overlay che blocca l'hover) + bottone "Attiva audio". **Vedi Decisioni aperte per lo swap a self-host.**
  - **Header** (`Header.astro`): **navbar floating a pillola** trasparente sull'hero (no barra classica), hover ambra, **menu mobile a tutto schermo**, **medaglia** (`aria-current`) sulla pagina attiva.
  - **Footer** (`Footer.astro`): **logo** bianco, **social** (Instagram/Facebook/YouTube/TikTok, URL reali dal sito attuale in `settings.social`), bottom-bar a 3 colonne (© / firma "Made with 🧡 by Mattia Pedone → mapped-dev.it" / note legali), voce **"Paintball VS Softair"** in `navigation.footerLinks` (fuori dal menu in alto).
...
1. **Completare la Home**: ridisegnare **Attività/Occasioni** in coerenza con la sezione arena (stesso linguaggio HUD/motion), poi Pacchetti/prezzi, Recensioni, Galleria, FAQ, Dove siamo, box Ranger, CTA finale.
...
- **VIDEO — self-host su Cloudflare R2** (come fatto per **campo-di-zucche**): quando il cliente avrà il file, si ospita su R2 e si sostituisce l'iframe YouTube con un `<video>` nativo (muted autoplay loop playsinline) — stesso layout, **zero chrome YouTube**, più leggero. Fino ad allora si usa l'embed YouTube ripulito (video attuale: `Zithahc6f0o`).
